...
<head>
    ...
    <script language="javascript" type="text/javascript">
        ...
        $(document).ready(function () {
            ...
            <?PHP if( $res['err_code'] ==1){  ?>
            msg=" <?PHP echo $res['msg'] ; ?>";
...
//            $.openAlter("<div style='text-align:left;'>网站全面升级，暂时停止新用户注册，预计在11月中旬恢复，多谢支持~</div>", "提示", { width: 250, height: 50 });
    ...
    <style type="text/css">
		...
		span.usericon{width:22px;height:22px;margin:8px auto}
    </style>
</head>
<body>
<form action="<?=$this->createUrl('user/login');?>" id="fm" method="post">
    <div class="login">
        ...
        <ul>
            <li><span class="fl usericon"><img width="100%" src="<?=M_IMG_URL?>user.png"/></span>
				...
				<span class="fr usericon" onclick="ClearTxt()"><img width="100%" src="<?=M_IMG_URL?>close4.png"/></span>
				
			</li>
            <li id="password1">
				<span class="fl usericon"><img width="100%" src="<?=M_IMG_URL?>lock.png"/></span>
				...
				<span class="fr usericon" onclick="ShowPassword()"><img width="100%" src="<?=M_IMG_URL?>close-eyes.png"/></span>
			</li>
            <li id="password2" style="display: none;">
				<span class="fl usericon"><img width="100%" src="<?=M_IMG_URL?>lock.png"/></span>
                <input type="text" class="login_2" placeholder="请输入密码" id="txtPwd2" maxlength="50">
				<span class="fr usericon" onclick="hidePassword()"><img width="100%" src="<?=M_IMG_URL?>eye.png"/></span>
            </li>
        </ul>
        <h3>
            <a href="javascript:void(0)" onclick="return ValiTest();" class="login_dl bg_blue" id="delu">登录</a></h3>
        <p>
