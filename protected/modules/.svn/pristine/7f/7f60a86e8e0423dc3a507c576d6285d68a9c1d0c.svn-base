...
<html lang="en">
	<head>
        ...
        <!--日期选择器-->
        <script language="javascript" type="text/javascript" src="<?php echo ASSET_URL;?>DatePicker/My97DatePicker/WdatePicker.js"></script>
	...
	<div class="container-fluid content">
		...
		<div class="main">
            ...
            <form onsubmit="return checkForm()" method="get" action="<?php echo $this->createUrl("taskcenter/tasklist")?>">
                <input name="act_start_time" type="text" onclick="WdatePicker()" class="text-box" value="<?php echo isset($_GET['act_start_time'])?$_GET['act_start_time']:"";?>" placeholder="开始时间" title="开始时间" readonly="readonly" style="cursor:pointer;"/>&nbsp;-&nbsp;
                <input name="act_stop_time" type="text" onclick="WdatePicker()" class="text-box" value="<?php echo isset($_GET['act_start_time'])?$_GET['act_stop_time']:"";?>" placeholder="结束时间" title="结束时间" readonly="readonly" style="cursor:pointer;"/>&nbsp;-&nbsp;
                <select name="status" class="text-box" style="height: 32px; line-height:30px; color:red; position: relative; top: 1px;">
                    ...
                    <option value="0" <?php echo @$_GET['status']==0 && isset($_GET['status'])?"selected='selected'":"";?>>正在进行</option>
                    <option value="1" <?php echo @$_GET['status']==1?"selected='selected'":"";?>>待发货</option>
                    <option value="2" <?php echo @$_GET['status']==2?"selected='selected'":"";?>>确认收货</option>
                    <option value="3" <?php echo @$_GET['status']==3?"selected='selected'":"";?>>商家确认</option>
                    <option value="4" <?php echo @$_GET['status']==4?"selected='selected'":"";?>>完成订单</option>
                    <option value="5" <?php echo @$_GET['status']==5?"selected='selected'":"";?>>待评价</option>
                    <option value="6" <?php echo @$_GET['status']==6?"selected='selected'":"";?>>已评价</option>
                    <option value="7" <?php echo @$_GET['status']==7?"selected='selected'":"";?>>已完成</option>
                </select>
                <input class="text-box" name="tasksn" id="tasksn" placeholder="任务编号" style="height: 32px; line-height:30px; " value="" />
                    ...
                    <table class="table table-hover">
                        ...
                        <?php
                            foreach($proInfo as $item){
                                $publishInfo=User::model()->findByPk($item->merchantid);//发布人信息
                                $taskerInfo=User::model()->findByPk($item->userid);//接手信息
                        ...
                        ?>
                            <tr>
                                <td><?php echo $item->addtime.'*'.$item->id;?></td>
                                <td>
                                    <?php
									    if($publishInfo){
                                ...
                                <td><span style="font-size: 14px; font-weight:bold; color:red;"><?php echo $taskmodel->price+$taskmodel->express;?></span>&nbsp;&nbsp;元</td>
                                <td><span style="font-size: 14px; font-weight:bold; color:red;"><?php echo $taskmodel->commission;?></span>&nbsp;&nbsp;元</td>
                                <td><span style="font-size: 14px; font-weight:bold; color:red;"><?php echo $item->commission;?></span>&nbsp;&nbsp;元</td>
                                <td>
                                    <?php
                                        switch ($item->status){
