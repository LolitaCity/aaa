...
class ArticleController extends Aclfilter
{
    ...
    */
    ...
    public function actionQuestionList(){
        parent::acl();
        $sql='SELECT id,question_type,statue,`order` FROM '.Yii::app()->db->tablePrefix.'_buyer_question ';
        $list=Yii::app()->db->createCommand($sql)->queryAll();

        $this->render('questionList',array('list'=>@$list));
    }
    public function actionQuestionAdd(){
        ...
        $detail=array();
        if (@$_GET['qid']){
           $sql="SELECT * FROM ".$tbprefix."_buyer_question WHERE id=$_GET[qid]";
           $detail=Yii::app()->db->createCommand($sql)->queryRow();
           $detail['question_content']=json_decode(urldecode($detail['question_content']));
        ...
        if(@$_POST['typename']){
           ...
           foreach ($_POST['question'] as $k=>$v){
               if (!empty($v)){
                   $content[$key]['title']=$v;
                   $content[$key]['answer'] = $_POST['content'][$k];
                   $key++;
               }
           }
            $content=urlencode(json_encode($content));
            if (@$_POST['qid']){
                $sql="UPDATE  ".$tbprefix."_buyer_question SET question_type='$_POST[typename]',question_content='$content',statue='$_POST[statue]',`order`='$_POST[order]' WHERE id=$_POST[qid]";
            }else{
                $sql="INSERT INTO ".$tbprefix."_buyer_question (question_type,question_content,statue,`order`)Values('$_POST[typename]','$content',$_POST[statue],'$_POST[order]')";
            }
            $res=Yii::app()->db->createCommand($sql)->execute();
           if ($res) {
               $this->redirect_message("操作成功","success",2,$this->createUrl('article/questionlist'));
           }else{
               $this->redirect_message("操作失败","fail",3,$this->createUrl('article/questionadd'));
           }

        }

        $this->render('questionAdd',array('detail'=>$detail));
    }
