...
<body>
                    ...
                    <table class="table table-hover">
                        <thead>
                            <tr>
                                ...
                                <th><div align="center">信用值</div></th>
                                <th><div align="center">淘气值</div></th>
                                <th><div align="center">威客圈截图</div></th>
                        ...
                        <tbody>
                            <?php
                                foreach($proInfo as $item){
									...
									$sql="SELECT * FROM ".Yii::app()->db->tablePrefix."_bindbuyer WHERE userid=".$item->userid;
									$buyermsg=Yii::app()->db->createCommand($sql)->queryAll();
									$buyermsg=@$buyermsg[0];
                                    $account_info=unserialize($buyermsg['account_info']);
                            ?>
                            <tr style="color:#666;">
                                ...
                                <td><div align="center"><?php echo @$item->platform==0?"淘宝":"京东";?></div></td>
                                <td><div align="center"><a class="checkImg" href="javascript:;" alt="<?php echo $account_info['xy_img'];?>"><?php echo $account_info['xy_val']?$account_info['xy_val'].'查看':'';?></a></div></td>
								<td><div align="center"><a class="checkImg" href="javascript:;" alt="<?php echo $account_info['tqz_img'];?>"><?php echo $account_info['tqz_val']?$account_info['tqz_val'].'查看':'';?></a></div></td>                               
								<td><div align="center"><a class="checkImg" href="javascript:;" alt="<?php echo @$buyermsg['wkqimg'];?>">查看</a></div></td>
                                <td><div align="center">
								<?php if(@$item->statue==1){?>
								<span style='color:green;'>通过</span>
								<?php }elseif(@$item->statue==0){?>
								<span style="color:red" onclick="if(confirm('确定审核通过？')) return check_statue(<?php echo @$item->id;?>); else return false;">审核</span>
								<?php }else{?>
								<span style='color:red;'>未通过</span>
								<?php }?>
								</div></td>
                                <td>
								<div align="center"><a onclick="if(confirm('您确定要删除吗？请确保该买号没有接过任务!删除后将无法恢复！')) return true; else return false;" href="<?php echo $this->createUrl('membercenter/wangwangdel',array('id'=>$item->id));?>">删除</a></div></td>
                            </tr>
                            <?php
                                }
                            ?>
	...
	<script>
	function check_statue(sid){
		$.ajax({
			type:"POST",
			url:"<?php echo $this->createUrl('membercenter/checkstatue');?>",
			data:{"id":sid},
			success:function(msg)
			{
				...
				{
					layer.confirm('<span style="color:red;">审核失败</span>', {
						btn: ['知道了'] //按钮
					},function(){
						location.reload();
					});
