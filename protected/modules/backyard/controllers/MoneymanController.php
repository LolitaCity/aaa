    ...
    class MoneymanController extends Aclfilter{
        ...
        */
        public function actionTxlist()
        {
            ...
            if(isset($_GET['txid']))
            {
				$txInfo=Outcash::model()->findByPk($_GET['txid']);

                $txInfo->status='已转账';//改变提现状态
                $txInfo->updatetime=time();//改变提现状态
                $txInfo->save();
            ...
            $criteria->order ="addtime desc";
            //分页开始
            $total = Outcash::model()->count($criteria);
            $pages = new CPagination($total);
            $pages->pageSize=8;//分页大小
            $pages->applyLimit($criteria);
            $proreg = Outcash::model()->findAll($criteria);
            //分页结束
		 ...
		 public function actionFinance()
        {
            ...
            $where='';$usersidstr='';
            if ($_GET['Username']){
                $users=User::model()->findAll("Username LIKE '%$_GET[Username]%' ");
                if (!empty($users)){
                    foreach ($users as $v){
                        $usersidstr.=$v->id.',';
                    }
                    $usersidstr=substr($usersidstr,0,-1);
                }
                $where=" userid IN ($usersidstr)";
            ...
            //分页开始
            $total = Cashlog::model()->count($criteria);
            $pages = new CPagination($total);
            $pages->pageSize=30;//分页大小
            $pages->applyLimit($criteria);
            
            $lists = Cashlog::model()->findAll($criteria);
            //分页结束
