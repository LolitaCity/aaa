        ...
        AcceptTask01 = function(){
           ...
           //判定任务的类型，进行赋值
            ...
            if(selectType=="0")
            {
                ...
                var att = {"outtime":outtime,"PlatformTypes":0,"TaskType":"无线端","TaskTypelen":length,"TaskPriceEnd":TaskPriceEnd,"FineTaskClassType":"销量任务","terminal":2}
               TaskCategory = 0;
              }else{
                ...
                var att = {"outtime":outtime,"PlatformTypes":0,"FGTaskType":"无线端","FGTaskTypelen":length,"FGTaskPriceEnd":TaskPriceEnd,"FineTaskClassType":"复购任务","terminal":2}
                TaskCategory = 1;
            }
            //发送请求
             $.ajax({
...
//                  if ($('#acceptTask2').length > 0) $('#acceptTask2').removeAttr("onclick");
                   ...
                   // GetQueueAcceptResult();
					...
					if (data.is_black_buyer==0 <?=PLATFORM == 'esx' ? '' : '&& qrnumber < 1';?>)
                    {
    ...
    function GetTask(TaskPrice,DownTaskPoint,TaskCategory){
        if(socket == null ) {
            ...
            socket.onopen = function (event) {
				//socket.send("hearbear_s<?PHP echo Yii::app()->user->getId();?>");
