...
<script>
    function openConfirm(message, title, obj, fun1, buttonText1, fun2, buttonText2) {
     ...
     //开始任务(预订单任务)
	    function BeginTask03(taskId,shebei) {
	    	console.log('BeginTask03');
	        var url='/task/Taskprelist/taskid/'+taskId+'/shebei/'+shebei+'.html';
...
<script type="text/javascript">
        $(document).ready(function () {
            ...
            $("#pretask_btn").attr("class", "off");
            showReservation($("#pretask_btn"));
            //禁止用F5键   
...
//点击任务状态跳转
        function SetTab(obj, taskStatus) {
         ...
         function SetTab01(obj, taskStatus) {
            ...
            $(obj).attr("class", "off");
            Msg(1);
            if(taskStatus == 'pretask')
            {
            	$('div.menudiv').load('/task/reservationList')
            }
        }
        
        function showReservation(obj)
        {
        	$(".menu>ul>li").removeClass("off");
            $(obj).attr("class", "off");
            var cc_load = layer.load();
            $('div.menudiv').load('/task/reservationList', '', function(){
            	layer.close(cc_load);
            });//记得把分页的div移入到div.menudiv中
        }
...
<form action="<?php echo $this->createUrl('task/taskmanage01');?>" enctype="multipart/form-data" id="fm" method="post">
    <div class="sj-fprw">
        ...
        <div class="tab1" id="tab1">
            ...
            <div class="menu">
                <ul>
                    <li id="pretask_btn"  onclick="showReservation(this, 'pretask')"  value="2" class="off">预订单/预约单</li>
                </ul>
            </div>
            <div class="menudiv">
