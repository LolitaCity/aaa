        ...
        <form action="<?=$this->createUrl('finance/paylist')?>" id="fm" method="get">
            <div class="fpgl-ss" style="margin-top: 0px;">
                ...
                <p style="line-height: 35px; margin-left:100px;">
                    支付时间：</p>
                <p>
                    <input class="laydate-icon" id="BeginDate" maxlength="16" name="BeginDate" onKeypress="javascript:if(event.keyCode == 32)event.returnValue = false;" onclick="laydate({istime: true, format: 'YYYY-MM-DD hh:mm'})" style="width:128px;height:28px;" type="text" value="<?php if ($serchword['BeginDate'])echo date('Y-m-d h:m',$serchword['BeginDate']);?>">~

                    <input class="laydate-icon" id="EndDate" maxlength="16" name="EndDate" onKeypress="javascript:if(event.keyCode == 32)event.returnValue = false;" onclick="laydate({istime: true, format: 'YYYY-MM-DD hh:mm'})" style="width:128px;height:28px;" type="text" value="<?php if ($serchword['EndDate'])echo date('Y-m-d h:m',$serchword['EndDate']);?>">
                </p>
                <p>
                </p>
                <p>
                    <input class="input-butto100-ls" type="button" onclick="Search()" value="查询">
            ...
            <div class="zjgl-right_2">
                <table>
                    ...
                    <tr>
                        ...
                        <td class="cff3430"><?=$v['orderprice']-($v['price']*$v['auction']+$v['express']*$v['auction']);?></td>
...
<script>
    function  Search() {
        ...
        window.location.href = '/finance/paylist?BeginDate=' + BeginDate + "&EndDate=" + EndDate;
    }
</script>
