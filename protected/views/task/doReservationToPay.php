...
<script type="text/javascript">
    ...
    function FouX()
    {
    	...
    	if (localStorage.getItem('hasStay' + $task_tag))
    	{
    		...
    		location.href = location.href + '#dishi';
    	}
    	else
    	{
...
<div class="container-fluid">
    ...
    <!--第头部标题:end-->
    <!--top 标题栏-->
    <?php if($taskinfo['how_search'] == 2):?>
    <div class="row alert alert-danger text-center" role="alert">您昨日已浏览完毕，请以新的关键词重新搜索商品（切勿直接进入），最终完成支付</div>
    <div class="row topk ">
    ...
    <div class="row toa ">
        ...
        <div class="col-sm-8 col-xs-12">
            ...
            <div class="col-sm-12 col-xs-12 kung kunr tran" style="font-size: 12px;">
                ...
                <div class="row ">
                    <div class="col-sm-5 col-xs-5 t_right tdoa">
                        <label><strong class="text-danger badge"><?php if ($taskinfo['intlet']==3){echo'新的APP淘口令';}else{echo '搜索新的关键字';}?>:</strong></label>
                    </div>
                    <div class="col-sm-7 col-xs-7 t_left tdoa">     
                    	 <label >
                    	 	 <?php if ($taskinfo['intlet']==3){ ?>            
                                <?=$taskinfo['new_keyword']?>                                                     
                             <?php }else{?>
                                    <?=$taskinfo['new_keyword']?>
                             <?php } ?>
                    	 </label>
                        ...
                        <div class="row">
                            <div id="auditShopDiv" class="col-md-12 col-sm-12 col-xs-12" >
                                ...
                                <div class="col-md-1 col-sm-1 col-xs-1" >
                                    <button type="button" class="btn btn-info" onclick="Confirm('<?=$taskinfo['taskid']?>','<?=$taskinfo['intlet']?>','<?php echo $this->createUrl('task/tasktestOne02');?>', 'pre')">验证</button>
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    <div class="row tdoa"  >
        ...
        <div class="col-sm-8 col-xs-12">
        	<div class="col-sm-12 col-xs-12 kung kunr tran">
                <strong class="badge">上传新的搜索图</strong>
            <div class="row tdoa">
                    ...
                    <?php if ($this -> getHandlePercent_collect($taskinfo['bookmark'], 'bookmark' . $taskinfo['id'])){?>
                        ...
                        <div class="row toa">
                            <div class="col-sm-12 col-xs-12 t_red">
                                <div class="checkbox"> <label><input type="checkbox" value="" class="step_four" name="has_collect_shop">收藏店铺</label> </div>
                            </div>
                        </div>
                        <!--控件文本：end-->
                    <?php } if ($this -> getHandlePercent_collect($taskinfo['talk'], 'talk' . $taskinfo['id'])){?>
                        ...
                        <div class="row tdoa">
                            <div class="col-sm-12 col-xs-12 t_red">
                                <div class="checkbox">
                                    <label>
                                        <input type="checkbox" value="" class="step_four" name="has_talk">买前咨询聊天(店铺旺旺对话几句，聊天内容严禁出现平台任务字眼)
                                    </label>
                                </div>
                            </div>
                        </div>
                    ...
                    if ($contrast_num != 'not')  //不是为 ‘不货比’
                    {
                        ...
                        <div class="row tdoa">
                            <div class="col-sm-12 col-xs-12 t_red">
                                <div class="checkbox">
                                    <label>
                                        <input type="checkbox" class="step_four" id="checkbox5" name="checkbox"  value="货比<?=@$num;?>家" >浏览<?=@$num;?>家同类店铺的一款同类商品
                                    </label>
                                </div>
                            </div>
                        </div>
                    ...
                    if ($contrast_num != 'net')
                    {
                        ...
                        <div class="row toa">
                            <div class="col-sm-12 col-xs-12 t_red">
                                <div class="checkbox">
                                    <label>
                                        <input type="checkbox" class="step_four" id="checkbox4" name="checkbox"  value="浏览店内<?=@$numdeep;?>款其他商品" > 浏览店内<?=@$numdeep;?>款其他商品
                                    </label>
                                </div>
                            </div>
                        </div>
...
<?php else:?>
	<div class="row alert alert-danger text-center" role="alert">您昨日已浏览完毕，
		<?php
    		switch($taskinfo['how_search'])
			{
				case 1:
					$how_search = '请从购物车直接进入';
					break;
				case 2: 
					$how_search = '从收藏夹直接进入';
					break;
	...
	<script type="text/javascript">
		var $task_tag = $('#task_tag').val();
		localStorage.setItem('imgUrl#lang1' + $task_tag, 1);
	</script>
	<div class="san" style="display: inline">
        <!--第三步-->
        <!--top 标题栏-->
        <div class="row topk ">
            <div class="col-sm-2 col-xs-0 " ></div>
            <div class="col-sm-8 col-xs-12 greeR kunr" >
                <h4 style="color: #fff;text-align: center;">第四步:核对商品信息</h4>
            </div>
            <div class="col-sm-2  col-xs-0" ></div>
        </div>
        <!--top 标题栏:end-->
        ...
        <div class="row tdoa">
            ...
            <div class="col-sm-8 col-xs-12">
                <div class="col-sm-12 col-xs-12 kunr kung " style="padding-bottom:3% ;">
                    <div class="row">
                        <div class="col-sm-12 col-xs-12 t_cen">
                            <h4 style="color: #000;text-align: center;"> 核对商品信息，确认无误后验证店铺名</h4>
                        </div>
                    </div>
					<div class="row tdoa">
						<div class="col-md-offset-4">
							<?php $taskinfo['commodity_image']= str_replace ( 'pk1172' , '18zang' ,  $taskinfo['commodity_image'] ); ?>
					...
					<div class="row doa">
						<div>
							<p class="text-center">
								<?=$taskinfo['intlet']==1||$taskinfo['intlet']==3||$taskinfo['intlet']==4||$taskinfo['intlet']==6 ? '验证店铺名' : '验证商品ID';?>
								<label for="" id="auditShopState"></label>
							</p>
						</div>
	                    <div class="col-sm-12 col-xs-12" >
	                        <div class="row">
	                            <div id="auditShopDiv" class="col-md-12 col-sm-12 col-xs-12" >
	                                <div class="col-md-11 col-sm-11 col-xs-11" >
	                                    <input type="text" class="form-control"  placeholder="<?php if ($taskinfo['intlet']==1||$taskinfo['intlet']==3||$taskinfo['intlet']==4||$taskinfo['intlet']==6){?> 请输入店铺名完成验证<?php }else{?>请输入商品ID完成验证<?php }?>"  id= "txtValidateValue" name="textfield"  checkstatus="
									<?php
									$coo = Yii::app ()->request->getCookies ();
									if ($coo ['comfirmcookie' . $taskinfo ['id']]->value) {
										echo 1;
	                                ...
	                                <div class="col-md-1 col-sm-1 col-xs-1" >
	                                    <button id="auditShopState" type="button" class="btn btn-info" onclick="Confirm('<?=$taskinfo['taskid']?>','<?=$taskinfo['intlet']?>', '<?php echo $this->createUrl('task/tasktestOne02');?>')">验证</button>
	                                </div>
	                            </div>
	                        </div>
	                    </div>
            ...
            <div class="col-sm-12 col-xs-12" style="font-size: 12px;">
                ...
                <div class="row ">
                    ...
                    <div class="col-sm-6 col-xs-6 t_left ">
                        <div class="checkbox" ?> 
                 			<label>
                 				<input class="step_four" type="checkbox" id="checkbox3" name="pro_model"  value="买前咨询聊天" />型号:<span class="shopanmeg" style="color:#000"><?=$taskinfo['modelname']?$taskinfo['modelname']:'默认';?></span>
                 			</label> 
                 		</div>
                    </div>
                </div>
                ...
                <div class="row ">
                    <div class="col-sm-6 col-xs-6 t_left ">
                        <div class="checkbox" ?> 
                 			<label>
                 				<input class="step_four" type="checkbox" id="checkbox3" name="pro_num"  value="买前咨询聊天" />件数:<span class="shopanmeg" style="color:#000"><?=$taskinfo['auction']?></span>
                 			</label> 
                 		</div>
                    </div>
                    <div class="col-sm-6 col-xs-6 t_left ">
                        <div class="checkbox" ?> 
                 			<label>
                 				<input class="step_four" type="checkbox" id="pay_money" name="pay_price"  value="买前咨询聊天" >付款金额:<span class="shopanmeg" style="color:#000"><?=$taskinfo['price']*$taskinfo['auction']+$taskinfo['express'];?></span>（包含快递费：<?=$taskinfo['express']?>元）
                 			</label> 
                 		</div>
                    </div>
                </div>                
    	  	    ...
    	  	    <div class="row">
    	  	    	 <div class="row text-center">
    	  	    	 	 ...
    	  	    	 	 <div class="col-sm-12 col-xs-12 toa">
    	  	    	 	 		<button type="button" class="btn btn-info" onclick="<?= $taskinfo['how_search'] == 2 ? "submitOldSearch(". $taskinfo['id'] . ", 1)" : "submitOldSearch(". $taskinfo['id'] . ")"?>" id="jindu" disabled="disabled" ><span id="tijiaoshijian" style="color: red;">0</span>分<span id="tijiaoshijian1" style="color: red;">0</span>秒后方可提交</button>
    	  	    	 	 </div>
    	  	         </div>
    	       </div>
          </div>
...
<script type="text/javascript">
  ...
  function SubmitAudit(ur){
  	 $is_use_helper = '<?=$is_use_helper;?>';
  	 if(!window.$payment && $is_use_helper)
  	 {
  	 ...
  	 var txtPayPrice = $use_helper ? $payment : $("#txtPayPrice").val();
  	 var orderNumber = $use_helper ? $ordersn : $("#txtPlatformOrderNumber").val();
   ...
   * 【直接进入商品】的提交方式
   */
  function submitOldSearch($pre_id, $how_search)
  {
  	   $use_helper = '<?=$is_use_helper;?>';
	   if(!window.$payment && $use_helper != '0')
	   {
	       return m_alter("请正确填入订单编号，并提交以获取订单信息");
	   }
	   var $post = {
	    		'pretask_id': $pre_id,
	    		'ordersn' : $use_helper != '0' ? $ordersn : $("#txtPlatformOrderNumber").val(),
	    		'pay_price': $use_helper != '0' ? $payment : $("#txtPayPrice").val(),
	    		'paytype': $('#selPayMethod').val(),
	   },
	   $load = layer.msg('提交数据中...', {
    	...
    	getTime(function(datas){
    		...
    		else
    		{
    			if ($how_search)
		    	{
		    		$post.new_tasktwoimg = $('#stu1').val();
		    		if ($post.new_tasktwoimg == '' || $post.new_tasktwoimg == 1)
		    		{
		    			return m_alter('请重新上传搜索截图');
		    		}
		    	}
    			if($('.step_four:checked').length != $('input[type=checkbox][class=step_four]').length)
				{
					return m_alter('请勾选对应按钮');
				}
				if (!$use_helper)
				{
					if(!window.localStorage.getItem('imgUrl#lang2' + $task_tag))
					{
						return m_alter('请上传订单截图');
					}
					if ($post.img_order == '')
					{
						return m_alter('请重新上传订单截图');
					}
					if ($post.pay_price == '')
					{
						return m_alter('请输入您实际支付的金额');
					}
				}
				if ($post.ordersn == '')
				{
					return m_alter('请输入订单编号');
				}
				if ($post.paytype == '')
				{
					return m_alter('请选择实际的支付方式');
				}
				if (CheckOrderNumber())
				{
					
					$.post('/task/submitOldSearch', $post, function(datas){
						layer.close($load);
						if (datas.status)
						{
