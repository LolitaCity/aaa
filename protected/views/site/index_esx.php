    ...
    function AcceptTask01(i)
    {
        ...
        if(i=="0")
        {
            ...
            var att = {"outtime":outtime,"PlatformTypes":PlatformTypes,"TaskType":TaskType,"TaskTypelen":TaskTypelen
                ,"TaskPriceEnd":TaskPriceEnd,"FineTaskClassType":"销量任务","terminal":1}
            var TaskCategory = 0;
        }
        else
        {
            ...
            var att = {"outtime":outtime,"FGPlatformTypes":PlatformTypes,"FGTaskType":TaskType,"FGTaskTypelen":TaskTypelen
                ,"FGTaskPriceEnd":TaskPriceEnd,"FineTaskClassType":"复购任务","terminal":1}
            var TaskCategory = 1;
        }
        $.ajax({
            type:'POST',
            ...
            success:function(data){
                ...
                }else if(data.acceptarr.err_code == 8){
                    layer.msg(data.acceptarr.msg, {icon: 7});
                    $('#acceptTask').attr({"onclick":"AcceptTask01(0)"})
                    $('#acceptTask1').attr({"onclick":"AcceptTask01(1)"})
                    $('#acceptTask').val("开始排队");
                }else {
                	if (data.acceptarr.msg != '正在接销量任务')
                	{
                ...
                if (isQueue=='True'){
                    ...
                    //钱   终端   任务类型
                    localStorage.clear();
					if (data.is_black_buyer==0 && data.qrnumber < 1)
                    {
        ...
        if(socket == null ) {
           ...
           // socket = new WebSocket("<?php echo WS_TASK;?>/Task?UserID=<?=Yii::app ()->user->getId () ?>&TaskPrice="+TaskPrice+"&DownTaskPoint="+DownTaskPoint+"&TaskCategory="+TaskCategory+" ");
            ...
            socket.onopen = function (event) {
				//socket.send("hearbear_p<?PHP echo Yii::app()->user->getId();?>");
            };
            // 监听Socket的关闭
...
//                      $("#loading1").hide();
//                      $("#loading2").show();
//                      $("#tack1").attr("onclick","");S
//                      $("#tack2").attr("onclick","");
//                      $("#bntOk1").css({'display':'block'});
//                      $("#bntOk").text("开始任务");
//                      $("#bntOk1").text("取消任务");
//                      $("#bntOk").attr("onclick","BeginTask01("+obj.Data.PTaskID+",'PC')");
//                      $("#bntOk1").attr("onclick","BeginTask02("+obj.Data.PTaskID+",'PC')");
//                      layer.confirm('店铺名称：'+obj.Data.ShopName+'<br/>任务说明：'+obj.Data.Task_Remark+'<br/>佣金：'+obj.Data.UTask_Commission,{
...
function GetTasktop(){
  ...
  //计时器(等待弹框:销量)
    function Getdengtime(minuteke2,secondke2,$taskid){
		...
		if (!window.layer_desc)
		{
			layer_desc = $layer.html();
		}
      	$('.layui-layer-content').html(layer_desc + '<br>若一直停留在当前页面，该任务倒计时' + minuteke2+"分"+secondke2+"秒" + '之后将自动释放');
    	secondke2--;
    ...
    function Getdengtimeyu(minuteke3,secondke3,$preid){
    	var $layer = $('.layui-layer-content');
		if (!window.layer_desc_pre)
		{
			layer_desc_pre = $layer.html();
		}
      	$('.layui-layer-content').html(layer_desc_pre + '<br>若一直停留在当前页面，该任务倒计时' + minuteke2+"分"+secondke2+"秒" + '之后将自动释放');
    	secondke3--;
