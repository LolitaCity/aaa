	...
	<div class="notice_box mt30">
		...
		<div>
			...
			<span style="line-height: 2;">
				3、信誉级别≥3心、注册时长≥1年、买号周平均信誉值≤4心、支付宝关联账户数≤2；&nbsp;&nbsp;</span><br> <span
...
?>
	...
	function checkdata(){
		...
		if (account_name == '') {
			...
			return false;}
		if (xy_val == '') {
			$.openAlter('信用值不能为空', '提示', { width: 250, height: 50 });
			return false;}
		if (xy_img=='') {
			$.openAlter('信用值截图不能为空', '提示', { width: 250, height: 50 });
			return false;}	
		if (tqz_val == '') {
			$.openAlter('淘气值不能为空', '提示', { width: 250, height: 50 });
			return false;}
		if (tqz_img == '') {
			$.openAlter('淘气值截图不能为空', '提示', { width: 250, height: 50 });
			return false;}	
		<?php if (PLATFORM == 'esx'):?>
			if (wkqimg=='') {
