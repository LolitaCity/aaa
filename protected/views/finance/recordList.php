...
<div class="sj-zjgl">
    ...
    <div class="zjgl-right">
        <!-- 搜索-->
        <form action="<?=$this->createUrl('/finance/recordList')?>" id="fm" method="get">
            ...
            <div class="fpgl-ss">
                <p style="line-height: 30px;">
                    ...
                    ~
                    <input class="laydate-icon" id="EndDate" maxlength="16" name="EndDate" onKeypress="javascript:if(event.keyCode == 32)event.returnValue = false;" onclick="laydate({istime: true, format: 'YYYY-MM-DD hh:mm'})" style="width:122px;height:28px;" type="text" value="<?php if ($searchword['EndDate'])echo date('Y-m-d h:m',$searchword['EndDate']);?>"></p>
                <p>
                    类型：<select class="select_160" id="FinanceType" name="FinanceType" style="width:144px;height:30px;"><option value="">请选择</option>
                        <option <?php if ($searchword['FinanceType']=='提现'){echo "selected=selected";}?> value="提现">提现</option>
                        <option <?php if ($searchword['FinanceType']=='任务佣金'){echo "selected=selected";}?>  value="任务佣金">任务佣金</option>
                    </select>
                </p>
                <p>
                    消费编号：<input class="select_215" id="FinanceKey" name="FinanceKey"  style="width:145px;height:28px;" type="text" value="<?=$searchword['FinanceKey']?>">
                </p>
                <p>
