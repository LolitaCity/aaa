...
<script type="text/javascript">
    ...
    function Search() {
        var PlatformOrderNumber =$("#PlatformOrderNumber").val();
        ...
        window.location.href = '/finance/transferdetailslist?PlatformOrderNumber='+PlatformOrderNumber+"&BeginDate="+BeginDate+"&EndDate="+EndDate;
    }
</script>
...
<div class="sj-zjgl">
    ...
    <form action="<?php echo $this->createUrl('finance/transferdetailslist');?>" id="fm" method="get">
            ...
            <form action="<?=$this->createUrl('finance/transferdetailslist')?>" id="fm" method="post">
                <div class="Cash_sz" style="margin-top: 20px">
                    <div class="Cash_state">
                        <label>
                            订单编号：</label>
                        <input class="input_140 right_25" id="PlatformOrderNumber" name="PlatformOrderNumber" type="text" value="<?=$searchword['PlatformOrderNumber'];?>">

                        <label style="margin-left: 30px">
                            到账时间：</label>
                        <input class="laydate-icon" id="BeginDate" maxlength="16" name="BeginDate" onKeypress="javascript:if(event.keyCode == 32)event.returnValue = false;" onclick="laydate({istime: true, format: 'YYYY-MM-DD hh:mm'})" style="width:122px;height:28px;" type="text" value="<?php if ($searchword['BeginDate'])echo date('Y-m-d h:m',$searchword['BeginDate']);?>">
                        <label>
                            ~</label>
                        <input class="laydate-icon" id="EndDate" maxlength="16" name="EndDate" onKeypress="javascript:if(event.keyCode == 32)event.returnValue = false;" onclick="laydate({istime: true, format: 'YYYY-MM-DD hh:mm'})" style="width:122px;height:28px;" type="text" value="<?php if ($searchword['EndDate'])echo date('Y-m-d h:m',$searchword['EndDate']);?>">
                        <a href="javascript:void(0)" onclick="Search()" class="b_4882f0 anniu">查询</a>
<!--                        <a onclick="Search()" class="b_4882f0 anniu">查询</a>-->
                    ...
                    <div class="Cash_table">
                        <table>
                            ...
                            <tr>
                                ...
                                <td> <?PHP
                                    $sql = "select bankName from zxjy_banklist where bankAccount = {$v['transferbank']} and userid = ".$v['merchantid'];
                                    $merbankname = Yii::app()->db->createCommand($sql)->queryRow();
                                    echo    $merbankname['bankName'].'&nbsp;&nbsp;&nbsp;&nbsp;尾号&nbsp;'.substr($v['transferbank'],-4);?></td>
                                <td><?=Toole::Substr_Cut_top($v['transfername']);?></td>
