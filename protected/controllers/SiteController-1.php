...
class SiteController extends Controller {
	...
	public function actionAcceptTask() {
		...
		if ($userinfo->Status) {
			...
			if ($_POST ['FineTaskClassType'] == '销量任务') {
				$quearr = Yii::app ()->redis->lrange ( 'userque', 0, 999 );
				...
				$quearr = Yii::app ()->redis->lrange ( 'fguserque', 0, 999 );
				foreach ( $quearr as $val ) {
					$uid = substr ( $val, 0, strpos ( $val, ',' ) );
					if ($uid == Yii::app ()->user->getId ())
						$kk = 1;
	...
	public function actionCancelQueue() {
		$quearr = Yii::app ()->redis->lrange ( 'userque', 0, 999 );
		$fgquearr = Yii::app ()->redis->lrange ( 'fguserque', 0, 999 );
	...
	public function actionGetQueAcceptRes() {
		...
		$queueall = Yii::app ()->redis->lrange ( 'userque', 0, 999 );
		$fgqueueall = Yii::app ()->redis->lrange ( 'fguserque', 0, 999 );
		$res = array ();
		// 判断传递过来的参数是复购还是销量任务
		...
		// 取出1-20的队列，看里面的排队时间有没有超过5分钟，超过就清队列，同时允许20个人进入队列领取任务
		...
		if (in_array ( Yii::app ()->user->getId (), $userstr )) {
			...
			if ($issame > 0 && ! empty ( $task ['taskmodelid'] ) && ! empty ( $task ['taskid'] ) && ! empty ( $task ['tasktimeid'] ) && $task ['tasktimeid'] != '' && $task ['tasktimeid'] != ' ' && $task ['taskid'] != '' && $task ['taskid'] != ' ' && $task ['taskmodelid'] != ' ' && $task ['taskmodelid'] != '') {
