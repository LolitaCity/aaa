    ...
    public function actionTasktestOne()
    {
        ...
        if (!empty ($_GET['taskid'])) {
            ...
            $sql = "SELECT ut.*, s.sid, s.shopname,t.intlet,tm.price,tm.express,tm.auction,tm.modelname,tm.car_img,
                    p.commodity_image as image,p.qrcode,p.commodity_image,p.c_goods,p.bookmark,p.talk,p.x_home,p.deep,p.commodity_id,p.through_train_1, p.through_train_2, p.through_train_3, p.through_train_4, p.app_img,
            ...
            if ((($taskinfo['addtime'] + 45 * 60) - time()) < 0) {
               ...
               // $ck = $this->updateTaskNum($taskinfo['taskid'], $taskinfo['tasktimeid'], $taskinfo['taskmodelid'], $taskinfo['id']);
			    $ck = $this->DeleteTask($_GET['taskid'],'TaskController/TasktestOne',2,'开始任务超时');
                if($ck){
			...
			if($buyer->statue == 0){
				$this->layout = false;
				//$ck = $this->updateTaskNum($taskinfo['taskid'], $taskinfo['tasktimeid'], $taskinfo['taskmodelid'], $taskinfo['id']);
				$ck = $this->DeleteTask($_GET['taskid'],'TaskController/TasktestOne',1,'您的淘宝账户未审核，请联系管理人员');
				if($ck){
						//$this->add_unnormaltask($taskinfo['id'], $uid, $taskinfo['merchantid'], $taskinfo['addtime'], $taskinfo['updatetime'], $taskinfo['taskid'], $taskinfo['shopid'], $taskinfo['taskmodelid'], $taskinfo['tasktimeid'], $taskinfo['status'], 'tasdcontrller/TaskExit');
            ...
            $this->render('Tasktestone', array(
                ...
                'rukou' => $rukou,
                'setfreetime' => $setfreetime,
				'setfreetimenow' =>$taskinfo['addtime'] + 45 * 60,
                'ketiam' => $num,
                'shebei' => $shebei,
...
//                'shebei' =>$shebei
    ...
    public function actionOrderall()
    {
        ...
        if ($_POST) {
           ...
           // if ((($taskinfo['addtime'] + 45 * 60) - time()) < 0) {
			//		$addtime = $taskinfo['addtime']+(20*60);
			//		$sql = "UPDATE zxjy_usertask SET `addtime`='$addtime' WHERE userid = {$uid} and taskid=" . $_POST[taskid];
			//		$resll = Yii::app()->db->createCommand($sql)->execute();
                //$this->layout = false;
                //$arr['err_code'] = 2;
                //$ck = $this->updateTaskNum($taskinfo['taskid'], $taskinfo['tasktimeid'], $taskinfo['taskmodelid'], $taskinfo['id']);
                //if($ck){
...
`orderprice`='$_POST[txtPayPrice]',`paytype`='$_POST[xia]'," . "updatetime=" . time() . ",`flag`=3, status=4 WHERE userid = {$uid} and taskid=" . $_POST[taskid];
                        $res = Yii::app()->db->createCommand($sql)->execute();
                        if ($res > 0) {
                            $this -> actionOrderMark($taskinfo['shopid'], $_POST['orderNumber']);  //插旗，为了兼容已过期商家对插旗的依赖性
                            $is_use_helper = $this -> isUseHelper($taskinfo['shopid']);
                            ...
                            $retry = 4;
                            while ((empty ($strJson) || !json_decode($strJson) -> IsOK) && $retry--)  //失败重试
                            {
                                $fp = fopen(BKC_URL . "?ActionTag=SendMoney&UserTaskID={$taskinfo['id']}" . $capital_limit, 'r');
                                $strJson = '';
                                while (!feof($fp)) {
    ...
    function updateTaskNum($taskid, $tasktimeid, $taskmodelid, $usertaskid)
    {
        ...
        //		}
        //		return false ;
		if($usertaskid < 1 || empty($usertaskid)){
            return false;
        }
    ...
    public function actionEvalTask()
    {
        ...
        if ($_GET ['usertaskid']) {
            ...
            $info ['imgcontent']= str_replace ( 'pk1172' , '18zang' ,  $info ['imgcontent'] ); 
        }
        if (!empty ($_POST)) {
            ...
            if (!empty($order_info['seller_userid']) && $order_info['api_expiration_time'] > time() && !in_array($order_info['seller_userid'], ['901575343', '1689361121']))
            {
                $curl_args = [
                    'tid' => $order_info['ordersn'],
                    'fields' => 'orders,buyer_rate,status',
                ];
                ...
                if (!$curl_error)  //curl无错误
                {
                    if (isset($tmpInfo -> code))  //有效请求
                    {
                        if ($tmpInfo -> code == 1)
                        {
                            if (in_array($tmpInfo -> data -> status, ['WAIT_SELLER_SEND_GOODS', 'WAIT_BUYER_CONFIRM_GOODS', 'TRADE_BUYER_SIGNED', 'TRADE_FINISHED', 'PAID_FORBID_CONSIGN']))  //判断订单状态
                            {
                                if (!$tmpInfo -> data -> orders[0] -> buyerRate)  //已评价
                                {
                            ...
                            Toole::show(0, '请求失败，[error]' . $tmpInfo -> code . ': ' . $tmpInfo -> msg);
                    }
                    else
                        Toole::show(0,  $tmpInfo -> msg);
                }
                else
        ...
        $this->layout = false;
        $sql = 'SELECT value FROM zxjy_system WHERE varname = "PlamGetTaskEvaluateMoney"';
        $info['commission_house'] = Yii::app ()->db->createCommand ( $sql )->queryScalar();
    ...
    public function actionEvalTaskList()
    {
        ...
        $this->render('evalTaskList', array(
            ...
            'search' => $search,
            'pages' => $pages,
            'commission_house' => Yii::app ()->db->createCommand ( $sql )->queryScalar(),
        ));
    }
...
//    }
    ...
    public function actionDoReservationToPay()
    {
        ...
        if (isset($_GET['taskid']))
        {
            ...
            $sql = 'SELECT p.*, t.intlet, tt.beginPay, tt.EndPay, tm.how_search, tm.new_keyword, pro.qrcode,pro.commodity_image,pro.c_goods,pro.bookmark,pro.talk,pro.x_home,pro.deep, pro.app_img,
    ...
    public function actionDoReservationToBrowse()
    {
        ...
        if (isset($_GET['taskid']))
        {
            ...
            $sql = 'SELECT p.*,s.shopname,t.intlet,tm.price,tm.express,tm.auction,tm.modelname,
                    pro.commodity_image as image,pro.qrcode,pro.commodity_image,pro.c_goods,pro.bookmark,pro.talk,pro.x_home,pro.deep,pro.commodity_id, pro.app_img,
     ...
     * @return mixed
     */
    public function _Lock($lock_name, $expire = 10)
    {
        $redis = Yii::app() -> redis;
        $is_lock = $redis -> setnx($lock_name, time() + $expire);
        if (!$is_lock)  //如果没有获取到锁，则判断锁是否失效，从而防止死锁
        {
            $lock_time = $redis -> get($lock_name);
            if (time() > $lock_time)  //锁失效，重新获取锁
            {
                $un_lock = $redis -> delete($lock_name);
                while (!$un_lock)  //预防释放锁失败
                {
                    $un_lock = $redis -> delete($lock_name);
                }
                $is_lock = $redis -> setnx($lock_name, time() + $expire);  //重新获取锁
            }
        }
        return $is_lock;
    }
    ...
    public function actionSubmitOldSearch()
    {
        ...
        if (isset($_POST['pretask_id']) && !in_array('', $_POST, true))
        {
            extract($_POST);
			$redis = Yii::app() -> redis;
            $lock_name = 'pre_submit_lock_' . $pretask_id;
            $is_lock  = $this -> _Lock($lock_name, 30); //获取锁
            if ($is_lock)  //获取锁成功
			{
				...
				if ($info)  //任务存在
				{
					if ($info['status'] == 1)  //任务状态为已浏览完毕
					{
						...
						if ($now > $info['beginPay'] && $now < $info['EndPay'])  //在规定的支付时间范围内
						{
							...
							if ($has_ordersn == 0)
							{
								if (abs($pay_price - ($info['price'] * $info['auction'] + $info['express'])) < LIMITEDAMOUNT)  //差额小于50
								{
									//准备更新pretask的语句
									...
									if ($db -> createCommand($sql) -> execute())
									{
										...
										if (Yii::app() -> db -> createCommand($sql) -> execute() !== false)
										{
											...
											$is_use_helper = $this -> isUseHelper($info['sid']);
											$this -> actionOrderMark($insert['shopid'], $ordersn);  //插旗，为了兼容商家对插旗的依赖性
											...
											if (empty ($strJson) == false)
											{
												...
												if ($jsonObject -> IsOK)
												{
													...
													Toole::show(1, '任务提交成功，但本金到账失败，[error]' . $jsonObject -> Description . '。我们将于明天凌晨为您补正，请耐心等待');
											}
											else
												Toole::show(1, '任务提交成功，但本佣到账失败，我们将于明天凌晨为您补正，请耐心等待');
										}
										else
                ...
                Toole::show(0, '请求过于频繁，请确认任务状态, 如果未重复提交，请' . ($redis -> get($lock_name) - time()) . '秒后再试');
        }
        else
    ...
    public function actionTaskprelist()
    {
        ...
        if (!empty ($_GET['id'])) {
            $shebei = $_GET ['shebei'];
            $id = $_GET ['id'];
            $sql = "SELECT pr.*,s.sid, s.shopname,t.intlet,tm.price,tm.express,tm.auction,tm.modelname,tm.handsel,tm.Payment,
                ...
                switch ($taskinfo['status']){
                    case  0:
                        ...
                        $this->render('Tasktestthree', array(
                            ...
                            'setfreetime' => $setfreetime,
                            'shebei' => $shebei,
                            'is_use_helper' => $this -> isUseHelper($taskinfo['sid']),
                        ));
                        exit();
                        break;
                    case  1:
                    ...
                    if($taskinfo['beginPay'] < time() && $taskinfo['EndPay'] >time() ){
                        ...
                        if($res){
                            ...
                            if( $usertask->save ()){
                                ...
                                $is_use_helper = $this->isUseHelper($pretask['shopid']);
                                $this->actionOrderMark($pretask['shopid'], $pretask['ordersn']);  //插旗，为了兼容商家对插旗的依赖性
                                if ($is_use_helper) //使用订单小助手，自动处理基础工单，插旗[放在本佣提现结果外面，防止提现失败导致小助手功能失效]
                                {
                                    $pretask['id'] = $usertask -> attributes['id'];
                                    $this->autoDealTrouble($pretask);
                                }
                                $capital_limit = $this->isUseHelper($pretask['shopid']) ? '&OrderPrice=' . ($_POST['txtPayPrice'] + $pretask['handsel']) : '';
                                $fp = fopen(BKC_URL . "?ActionTag=SendMoney&UserTaskID={$chausertask['id']}" . $capital_limit, 'r');
                                $strJson = '';
                                while (!feof($fp)) {
                                    $line = fgets($fp, 1024);
                                    $strJson = $strJson . $line;
...
//                    }
	...
	/**
	*  获取小助手授权token,为应对新版本API对多端登陆代理号的限制，
	*/
	public function actionGetTopToken()
    {
        $accessToken_cache_name = 'top_token';
        $redis = Yii::app() -> redis;
        //缓存accessToken
        if (!$redis -> get($accessToken_cache_name))
        {
            ...
            $redis -> set($accessToken_cache_name, serialize($auth_infos), $auth_infos['tmpInfo'] -> data -> expireIn - 1);
        }
        $cache_value = unserialize($redis -> get($accessToken_cache_name));
        $auth_infos = $cache_value ? $cache_value : [
            "tmpInfo：" => null,
            'curl_error' => '获取token无效',
        ];
        exit(json_encode($auth_infos['tmpInfo']));
    }

    /**
     *  请求服务接口，获取信息
     * @param $shopid  本地店铺ID，用以获取商家在淘宝后台的信息
     * @param $accessToken  授权时获取到的accessToken
     * @param $curl_args  请求参数
     * @param $top_api  淘宝开放平台对应的API名称
     * @param $retries  curl重试次数
     */
    public function curlApi($seller_userid, $curl_args, $top_api = 'getTradeFull', $retries = 4)
    {
		/**
        $accessToken_cache_name = 'top_token';
        $redis = Yii::app() -> redis;
        //缓存accessToken
        if (!$redis -> get($accessToken_cache_name) || true)
        {
            $now = date('Y-m-d H:i:s');
            $args = 'appid=' . TOP_APPID . '&secret=' . strtoupper(MD5(TOP_SECRET)) . '&timestamp=' . $now;
            $sign = strtoupper(MD5($args));
            $auth_url = 'http://top.mishion.com/api/top/apiLogin?appid=' . TOP_APPID . '&sign=' . $sign . '&timestamp=' . urlencode($now);
            $auth_infos = $this -> _curl($auth_url);  //获取accessToken
            $redis -> set($accessToken_cache_name, serialize($auth_infos), $auth_infos['tmpInfo'] -> data -> expireIn - 5);
        }
        $cache_value = unserialize($redis -> get($accessToken_cache_name));
        $auth_infos = $cache_value ? $cache_value : [
            "tmpInfo：" => null,
		...
		*/
        ...
        if (empty($auth_infos['curl_error']) && $auth_infos['tmpInfo'] -> code == 1)
        {
            ...
            $original_args = array_merge([
                'Pid' => 8015,
                'Key' => 'RzysIaWRonkeQ5wttdyyUYJP9iulweMQ',
            ], $curl_args);
    ...
    public function actionGetOrderInfos()
    {
        if (isset($_POST['shopid']) && isset($_POST['ordersn']) && isset($_POST['proid']))
        {
            ...
            if (!empty($seller_userid))
            {
                $curl_args = [
                    'tid' => $_POST['ordersn'],
                    'fields' => 'payment,title,created,pay_time,tid,buyer_nick,credit_card_fee,status,orders,num_iid,step_trade_status,step_paid_fee',
                ];
                ...
                if (!$curl_error)  //curl无错误
                {
                    if (isset($tmpInfo -> code))  //有效请求
                    {
                        if ($tmpInfo -> code == 1)
                        {
                            $tasktype = isset($_POST['tasktype']) ? $_POST['tasktype'] : 1;
                            if (in_array($tmpInfo -> data -> status, ['WAIT_SELLER_SEND_GOODS', 'WAIT_BUYER_CONFIRM_GOODS', 'TRADE_BUYER_SIGNED', 'TRADE_FINISHED', 'PAID_FORBID_CONSIGN']) || ($tasktype == 4 && in_array($tmpInfo -> data -> stepTradeStatus, ['FRONT_PAID_FINAL_NOPAID', 'FRONT_PAID_FINAL_PAID'])))  //判断订单状态
                            {
                                if (date('Y-m-d', strtotime($tmpInfo -> data -> payTime)) == date('Y-m-d') || $tasktype == 4 || true)  //当天付款订单
                                {
                                    ...
                                    if (empty($this_pro_is_new) || $tmpInfo -> data -> orders[0] -> numIid == $this_pro_is_new)  //买错商品
                                    {
                                        Yii::app() -> redis -> set('order_info_' . $_POST['ordersn'], serialize($tmpInfo -> data), 0);
                                        Toole::show(1, '', $tmpInfo -> data);
                                    }
                                    else
                                        Toole::show(0, '订单商品[ID:' . $tmpInfo -> data -> orders[0] -> numIid . ']与商家指定[ID: ' . $this_pro_is_new . ']的不一致，即拍错商品，无法获取信息。且勿重新再拍，否则将冻结买号，扣除200积分，扣除双倍佣金');
                                }
                                else
    ...
    public  function actionOrderMark($shopid, $ordersn)
    {
        ...
        if (!empty($shop_infos) && $shop_infos['is_mark'])  //有对应店铺信息，且设置了插旗
        {
            if ($shop_infos['api_expiration_time'] > time())
            {
                $curl_args = [
                    'tid' => $ordersn,
                    //'memo' => urlencode($shop_infos['mark_comment']),
					'memo' => $shop_infos['mark_comment'],
                    'flag' => $shop_infos['mark_val'],
                ];
                ...
                if (!$curl_error)  //curl无错误
                {
                    if (isset($tmpInfo -> code))  //有效请求
                    {
                        if ($tmpInfo -> code != 1)
                        {
                            $this -> actionMarkErrorHandle($ordersn, $tmpInfo -> code . ': ' . $tmpInfo -> msg);
                        }
                    }
                    else
                        $this -> actionMarkErrorHandle($ordersn, $tmpInfo -> msg);
                }
                else
...
//            }
    ...
    public function deductAndReturn($task_infos, $penaltymoney, $cash_remark, $need_create_schedual = true, $penaltyscore = 0, $schedual_type = [], $all_return = false, $fromUser = 'userid', $toUser = 'merchantid', $seriousness = 0)
    {
        ...
        if ($rs_json && $rs_json -> IsOK)  //扣返成功，将工单置为处理完成
        {
            if ($all_return)
            {
                $grad_type = $task_infos['tasktype'] == 1 ? 'commission_seller' : 'precommission_seller';  //梯度类型
                $sql = "SELECT value FROM zxjy_system WHERE varname = '$grad_type'";
                $gradient_info = $this -> db -> createCommand($sql) -> queryRow();  //梯度信息
                $gard = explode('|', $gradient_info['value']);
                $gradient = [];
                $money = $task_infos['price'] * $task_infos['auction'] + $task_infos['express'];  //对应任务的任务价格
                $commission = $penaltymoney;  //初始化佣金
                foreach ($gard as $k => $v)
                {
                    $temporary = explode('=', $v);
                    $start = $k == 0 ? 0 : explode('=', $gard[$k-1])[0];
                    if ($start <= $money && $money < $temporary[0])
                    {
                        $commission = $temporary[1];
                        break;
                    }
                }
                if (PLATFORM == 'esx')  //如果是二师兄，则进行全返
                {
                    $return_data = json_encode([
                        'UserID' => $task_infos [$toUser],
                        "Money" => $commission - $penaltymoney,
    ...
    {
        if (!Yii::app() -> redis -> get('is_use_helper') || true)
        {
            $sql = 'SELECT value from zxjy_system WHERE id = 104';
            Yii::app() -> redis -> set('is_use_helper', Yii::app() ->db -> createCommand($sql) -> queryScalar(), 0);
        }
        $is_use_helper = Yii::app() -> redis -> get('is_use_helper');
        $sql = 'SELECT COUNT(1) as has FROM zxjy_shop WHERE seller_userid IS NOT NULL AND api_expiration_time > UNIX_TIMESTAMP() AND sid = ' . $shopid;   //判断该店铺小助手是否有效
        if ($is_use_helper == 2 && !$this -> db -> createCommand($sql) -> queryScalar() || (in_array($shopid, [3444, 3047]) && PLATFORM == 'xkq'))  //如果是兼容模式，则判断商家是否存在未绑定或过期的店铺
        {
            $is_use_helper = 0;
        }
        return $is_use_helper;
    }
...
//        $rs_data = $this -> consoleCurlHandle('UserTransAccounts_Display', $post_data);
        $task_infos = [];
        $task_infos['tasktype'] == 1;

        var_dump(urlencode('哈'));
    }
     ...
     * 获取做任务的时候需要显示的图片
     */
    function getProductImg($taskinfo)
    {
        $url_img = $taskinfo['commodity_image'];
		$url_img = str_replace ( 'pk1172' , '18zang' ,  $taskinfo['commodity_image'] );
        if ($taskinfo['intlet'] == 1 && !empty($taskinfo['app_img']))
        {
            $url_img = str_replace ( 'pk1172' , '18zang' ,  $taskinfo['app_img'] );
        }
        elseif($taskinfo['car_img'] != 0)
        {
            $through_train_img = 'through_train_' . $taskinfo['car_img'];
            $url_img = str_replace ( 'pk1172' , '18zang' ,  $taskinfo[$through_train_img] );
        }
        return $url_img;
    }
    ...
    function actionDelall()
    {
        ...
        if($uid){
            if($_POST){
                ...
                if(!empty($usertask)){
					//$ck = $this->DeleteTask($taskid,'TaskController/TaskExit',3,"接到后15分钟没操作");
                    //$url = BKC_URL . "?ActionTag=ClearUserTask&UserTaskID={$usertaskid}";
                    //$rr = Toole::curl_get($url);
                    //$jsonObject = json_decode($rr);
                    //if ($rr) {
        ...
        if($uid){
            if($_POST){
                ...
                if($pretask){
                    ...
                    $rr = Toole::curl_get($url);
                    $jsonObject = json_decode($rr);
                    $ck = $this->DeleteTaskyu($id,'TaskController/Delallyu',3,'接到后15分钟没操作');
                    if ($ck) {
                        $arr['err_code'] = 1;
                        $arr['msg'] = "任务被释放";
                        echo json_encode($arr);
	...
	// 删除任务销量(新)
    function DeleteTask($taskid,$dofun,$type,$Remark)
    {
        $Usertask = Usertask::model()->find('taskid=' . $taskid. ' AND userid=' . Yii::app()->user->getId());
        if($Usertask->id < 1 ){
            return false;
            exit();
        }
        $url = BKC_URL . "?ActionTag=ClearUserTask&UserTaskID={$Usertask->id }";
        $rr = Toole::curl_get($url);
        $jsonObject = json_decode($rr);
        if ($rr) {
            $new_id = $this->add_unnormaltask($_POST ['taskid'], Yii::app()->user->getId(), $Usertask->merchantid, $Usertask->addtime, $Usertask->updatetime, $Usertask->taskid, $Usertask->shopid, $Usertask->taskmodelid, $Usertask->tasktimeid, $Usertask->status, $dofun);
            $userlog = new Userlog ();
            $userlog->userid = Yii::app()->user->getId();
            $userlog->logtype = '退出任务';
            $userlog->content = $Remark;
            $userlog->addtime = time();
            $userlog->unnormaltaskid = $new_id;
            $userlog->type =$type;
            $userlog->save();
	...
	// 删除任务预约预定(新)
    function DeleteTaskyu($preid,$dofun,$type,$Remark)
    {
        $uid =  Yii::app()->user->getId();
        if($preid < 1 ){
            return false;
        ...
        $sql = "select * from  zxjy_pretask  WHERE userid = {$uid} and  id=" . $preid ;
        $pretask = Yii::app()->db->createCommand($sql)->queryRow();
        $url = BKC_URL . "?ActionTag=ClearUserTask&UserTaskID={$preid}";
        $rr = Toole::curl_get($url);
        $jsonObject = json_decode($rr);
        if ($rr) {
            $new_id = $this->add_unnormaltask($_POST ['taskid'], Yii::app()->user->getId(), $pretask['merchantid'], $pretask['addtime'], $pretask['updatetime'], $pretask['taskid'],$pretask['shopid'] ,$pretask['taskmodelid'] , $pretask['tasktimeid'], $pretask['status'], $dofun);
            $userlog = new Userlog ();
            $userlog->userid = Yii::app()->user->getId();
            $userlog->logtype = '退出任务';
            $userlog->content = $Remark;
