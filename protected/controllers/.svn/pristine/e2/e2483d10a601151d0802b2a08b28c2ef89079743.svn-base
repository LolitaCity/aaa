...
`orderprice`='$_POST[txtPayPrice]',`paytype`='$_POST[xia]'," . "updatetime=" . time() . ",`flag`=3, status=4 WHERE userid = {$uid} and taskid=" . $_POST[taskid];
    ...
    public function actionEvalTask()
    {
        ...
        if (!empty ($_POST)) {
            ...
            if (!empty($order_info['seller_userid']) && $order_info['api_expiration_time'] > time())
            {
...
//    }
    ...
    public function actionSubmitOldSearch()
    {
        ...
        if (isset($_POST['pretask_id']) && !in_array('', $_POST, true))
        {
            extract($_POST);
            $redis = Yii::app() -> redis;
            $cache_name = 'submit_pre_time_' . $pretask_id;
            $r  = $redis -> get($cache_name);
            if (!$redis -> get($cache_name))
            {
                $redis -> set($cache_name, time(), 10);
                ...
                Toole::show(0, '请求过于频繁，请确认任务状态, 如果未重复提交，请' . ($redis -> get($cache_name) + 10 - time()) . '秒后再试');
        }
        else
...
//            }
    ...
    function  isUseHelper($shopid)
    {
        if (!Yii::app() -> redis -> get('is_use_helper') || true)
        {
            ...
            Yii::app() -> redis -> set('is_use_helper', Yii::app() ->db -> createCommand($sql) -> queryScalar(), 60);
        }
        ...
        if ($is_use_helper == 2 && !$this -> db -> createCommand($sql) -> queryScalar())  //如果是兼容模式，则判断商家是否存在未绑定或过期的店铺
        {
            $is_use_helper = 0;
        }
