...
//		$outcashlist = Outcash::model ()->findAll ( 'userid=' . $uID .' order by addtime desc ');
		...
		if (! empty ( $_POST ['money'] ) && is_numeric ( $_POST ['money'] )) {
			...
			if ($_POST ['money'] > $usermoney) {
				$res ['err_code'] = 1;
				$res ['msg'] = '提现金币不足，提现失败'.$usermoney;
				echo json_encode ( $res );
				exit ();
			}
		...
		//是否存在全额提现的记录
        $sql = "SELECT * FROM `zxjy_outcashfull` where  userid = {$uID} order by id desc; ";
        $outcashfull =  Yii::app()->db->createCommand($sql)->queryAll();
		$this->render ( 'takeOutCash', array (
				'banklist' => $banklist,
				'userinfo' => $userinfo,
				'outcashList' => $outcashlist,
				'outcashfull'=> $outcashfull,
                'pages' => $pages,
				'systemset' => $systemset,
				'danbaojin' => $danbaojin,
				'ree' => $ree,
	...
	//本金体现押金全提
    public function actionTakeoutcashall(){
       $uid = Yii::app ()->user->getId () ;
        $mark = 'out_' . $uid;
        $redis = Yii::app() -> redis;
//        if (time() - $redis -> get($mark) < 60 * 10)  //请求间隔小于10分钟直接返回
...
//        }
        ...
        $usermoney = Yii::app ()->db->createCommand ( $sql )->queryScalar ();
        $userinfo = User::model ()->findByPk ( $uid );
        $usermoneyall = $userinfo->Money;
        $sql = "select bankName from zxjy_banklist where userid = {$uid} and bankaccount={$_POST ['bankaccount']}";
        $bankAccount =  Yii::app ()->db->createCommand ( $sql )->queryScalar();
		...
		$sql = "SELECT * FROM `zxjy_outcashfull` where  userid = {$uid}; ";
        $outcashfull =  Yii::app()->db->createCommand($sql)->queryAll();
        if($outcashfull){
            $res ['err_code'] = 2;
            $res ['msg'] = '已经存在全额提现工单';
        ...
        //最后接单时间
        $sql = "select addtime from zxjy_usertask where userid = {$uid} order by addtime desc LIMIT 1,1;";
        $receipt = Yii::app()->db->createCommand($sql)->queryScalar();
        if ($userinfo->alloutcash !=1) {
            $res ['err_code'] = 2;
            $res ['msg'] = '联系客服开通全额提现！';
            echo json_encode ( $res );
            exit ();
        ...
        if(empty($receipt) || $receipt ==""){
            //$res ['err_code'] = 2;
           // $res ['msg'] = '未做过任务！';
            //echo json_encode ( $res );
           // exit ();
        }else{
          if($receipt+(2592000) > time() ){
              $res ['err_code'] = 2;
              $res ['msg'] = '30天内有做过任务！，无法全额提现！';
              echo json_encode ( $res );
              exit ();
          }
        }
        ...
        if (empty($usermoney) || $usermoney ==0) {
            $res ['err_code'] = 2;
            $res ['msg'] = '无法全额体现需要联系客服！';
            echo json_encode ( $res );
            exit ();
        }
        if(( round($usermoney-$usermoneyall,0))>0){
            $rea['err_code'] = 2;
            $res ['msg'] = '无法全额体现需要联系客服！';
            echo json_encode($rea);
            exit();
		...
		$sql = "UPDATE zxjy_user SET Status =1, ispromoter = 0 ,iscommission =0 WHERE id='{$uid}'";
		$rr = Yii::app ()->db->createCommand ( $sql )->execute ();
		$outcashfull = new Outcashfull();
		$outcashfull->money = $usermoney;
		$outcashfull->cashtype = 0;
		$outcashfull->bankaccount=$_POST ['bankaccount'];
		$outcashfull->addtime = time();
		$outcashfull->status  ="待确认";
		$outcashfull->userid = $uid;
		$outcashfull->name = $userinfo->TrueName;
        $outcashfull->bank =$bankAccount;
		$outcashfull->save();
		if($rr){
			$rea['err_code'] = 2;
			$rea['msg'] ="你成功提现本金，你的账号将被冻结";
			echo json_encode($rea);
			exit();
		}else{
			$rea['err_code'] = 2;
			$rea['msg'] ="你成功提现本金，你的账号未被冻结";
			echo json_encode($rea);
			exit();
		}
		$res ['err_code'] = 0;
	...
	public function actionSellerOutCash() {
		...
		$sql = 'SELECT ut.ordersn AS osn,ut.orderprice,ut.id AS utaskid,ut.status as ut,b.bankAccount,b.bankName,t.*, IF(c.beizhu IS NOT NULL AND t.id IS NULL, 1, 0) as can_manual FROM zxjy_usertask AS ut 
                LEFT JOIN zxjy_transfercash AS t  ON t.usertaskid=ut.id 
                LEFT JOIN zxjy_banklist AS b ON b.id =  t.bankid 
                LEFT JOIN zxjy_cashlog as c on c.usertaskid = ut.id AND beizhu = "完成任务获取"
                WHERE ut.userid= ' . Yii::app ()->user->getId () . ' AND ut.status>0   ORDER BY ut.addtime DESC ';
		...
		if ($_GET ['usertaskid']) {
            ...
            if (!empty($bank))
            {
                if (empty ( $transfercash)) {
                    ...
                    $transfer->buyerid = $usertask -> buyerid;
                    ...
                    $transfer->money = $usertask->orderprice;
                    $transfer->bankid = $bank->id;
                    $transfer->truename = $bank->truename;
                    if ($transfer->save ()) {
                        Toole::show(1, '申请提现成功');
                    }
                }
                else
                    Toole::show(0, '该笔任务已经申请本金提现，请勿重复操作');
            }
            else
                Toole::show(0, '请设置默认银行卡后再操作');
		}
		else
		    Toole::show(0, '参数错误');
	}
...
//                        Yii::app()->redis->select(0);
 ...
 //
                       ...
                       // $address =Toole:: Removalofspace($_POST['address']);
                        if(Toole::containcharacter($branch,"支行",-2)){
                           // $sql = "update zxjy_banklist set `bankName` = '{$_POST ['BankName']}',`bankAccount` ='{$bankAccount}',`bankAddress` = '{$address}',subbranch ='$branch' where id ={$_POST ['id']}";
							$sql = "update zxjy_banklist set `bankName` = '{$_POST ['BankName']}',subbranch ='$branch' where id ={$_POST ['id']}";
	...
	public function actionPayList() {
		...
		$sql = 'SELECT ut.tasksn,ut.updatetime,ut.id AS usertaskid,ut.ordersn,ut.orderprice,s.shopname,tm.price,tm.express,tm.auction,w.wangwang FROM ' . $tbprefix . '_usertask AS ut ' . ' LEFT JOIN ' . $tbprefix . '_shop AS s ON ut.shopid=s.sid LEFT JOIN ' . $tbprefix . '_blindwangwang AS w ON ut.userid=w.userid LEFT JOIN ' . $tbprefix . '_taskmodel AS tm ' . ' ON tm.id=ut.taskmodelid WHERE ut.userid=' . Yii::app ()->user->getId () . " $where " .' order by ut.updatetime desc ';
